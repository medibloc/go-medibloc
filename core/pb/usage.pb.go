...
func (*TxTimestamp) Descriptor() ([]byte, []int) { return fileDescriptorUsage, []int{1} }
...
func (m *TxTimestamp) Marshal() (dAtA []byte, err error) {
	size := m.Size()
	dAtA = make([]byte, size)
	n, err := m.MarshalTo(dAtA)
	if err != nil {
...
func (m *TxTimestamp) MarshalTo(dAtA []byte) (int, error) {
	var i int
	_ = i
	var l int
	_ = l
	if len(m.Hash) > 0 {
		dAtA[i] = 0xa
		i++
		i = encodeVarintUsage(dAtA, i, uint64(len(m.Hash)))
...
func (m *Usage) Unmarshal(dAtA []byte) error {
	...
	for iNdEx < l {
		...
		case 1:
			...
			m.Timestamps = append(m.Timestamps, &TxTimestamp{})
...
func (m *TxTimestamp) Unmarshal(dAtA []byte) error {
	l := len(dAtA)
	iNdEx := 0
	for iNdEx < l {
		preIndex := iNdEx
		...
		wireType := int(wire & 0x7)
		if wireType == 4 {
			return fmt.Errorf("proto: TxTimestamp: wiretype end group for non-group")
		}
		if fieldNum <= 0 {
			return fmt.Errorf("proto: TxTimestamp: illegal tag %d (wire type %d)", fieldNum, wire)
		}
		switch fieldNum {
		case 1:
			if wireType != 2 {
...
var fileDescriptorUsage = []byte{
	...
	0x32, 0x5c, 0x9c, 0x70, 0x0d, 0x12, 0x4c, 0x0a, 0x8c, 0x1a, 0xcc, 0x41, 0x08, 0x01, 0x27, 0x9e,
	0x13, 0x8f, 0xe4, 0x18, 0x2f, 0x3c, 0x92, 0x63, 0x7c, 0xf0, 0x48, 0x8e, 0x31, 0x89, 0x0d, 0xec,
	0x36, 0x63, 0x40, 0x00, 0x00, 0x00, 0xff, 0xff, 0x10, 0xe9, 0x39, 0x06, 0xaa, 0x00, 0x00, 0x00,
}
