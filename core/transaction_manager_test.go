...
// 		actual = node.Med.TransactionManager().Next()
...
// 	for actual == nil || !bytes.Equal(tx.Hash(), actual.Hash()) {
// 		require.True(t, time.Now().Sub(startTime) < time.Duration(5*time.Second))
// 		actual = seed.Med.TransactionManager().Next()
// 		time.Sleep(10 * time.Millisecond)
// 	}
// }
//
...
// 	err := tm.Push(wrongChainIDTx)
// 	assert.Equal(t, core.ErrInvalidTxChainID, err)
//
// 	// Wrong hash
...
// 	err = tm.Push(wrongHashTx)
// 	assert.Equal(t, core.ErrInvalidTransactionHash, err)
//
// 	// No signature
// 	noSignTx := randomTb.Sign([]byte{}).Build()
// 	err = tm.Push(noSignTx)
// 	assert.Equal(t, core.ErrTransactionSignatureNotExist, err)
//
// 	// // Invalid signature
...
//
// 	err := seedTm.PushAndBroadcast(randomTx2)
// 	require.Nil(t, err)
//
...
// 	}
//
// 	err = seedTm.PushAndBroadcast(randomTx1)
// 	require.Nil(t, err)
//
// 	startTime := time.Now()
