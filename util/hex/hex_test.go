...
func TestEncode(t *testing.T) {
	for _, test := range encodeBytesTests {
		...
		assert.Equalf(t, enc, test.want, "input %x: wrong encoding %s", test.input, enc)
	}
}
...
func TestDecode(t *testing.T) {
	for _, test := range decodeBytesTests {
		...
		assert.Equalf(t, test.want.([]byte), dec, "input %s: value mismatch: got %x, want %x", test.input, dec, test.want)
	}
}
...
func TestDecodeBig(t *testing.T) {
	for _, test := range decodeBigTests {
		...
		assert.Zerof(t, dec.Cmp(test.want.(*big.Int)), "input %s: value mismatch: got %x, want %x", test.input, dec, test.want)
	}
}
...
func TestDecodeUint64(t *testing.T) {
	for _, test := range decodeUint64Tests {
		...
		assert.Equalf(t, dec, test.want.(uint64), "input %s: value mismatch: got %x, want %x", test.input, dec, test.want)
	}
}
